...
<body>
  ...
  <div class="container mx-auto px-16 py-8">
    <a href="/customers/create"  class="px-3 py-2  bg-teal-300 text-white rounded-lg hover:bg-teal-600 transition">Create Customer</a>
      ...
      <table class="min-w-full bg-white border border-gray-200 rounded-lg shadow">
        ...
        <tbody>
         ...
         <tr class="hover:bg-gray-50">
           ...
           <td class="py-2 px-4 border-b">{{ $c->address }}</td>
        
           <td>
            <form action="/customers/{{ $c->id }}" method="POST" onsubmit="return confirm('Are you sure?')"  >
                @csrf
                @method('DELETE')
                <button type="submit"  class="px-3 py-1 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">Delete</button>
                <a href="/customers/{{ $c->id }}/edit"  class="px-3 py-1 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition">Update</a>
            </form>
           </td>
         </tr>
